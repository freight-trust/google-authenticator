...
static void usage(void) {
  puts("google-authenticator [<options>]\n"
       ...
       " -d, --disallow-reuse     Disallow reuse of previously used TOTP "
       "tokens\n"
       " -D, --allow-reuse        Allow reuse of previously used TOTP tokens\n"
       " -f, --force              Write file without first confirming with "
       "user\n"
...
int main(int argc, char *argv[]) {
  ...
  for (;;) {
    ...
    static struct option options[] = {
        {"help", 0, 0, 'h'},           {"counter-based", 0, 0, 'c'},
        {"time-based", 0, 0, 't'},     {"disallow-reuse", 0, 0, 'd'},
        {"allow-reuse", 0, 0, 'D'},    {"force", 0, 0, 'f'},
        {"label", 1, 0, 'l'},          {"issuer", 1, 0, 'i'},
        {"quiet", 0, 0, 'q'},          {"qr-mode", 1, 0, 'Q'},
        {"rate-limit", 1, 0, 'r'},     {"rate-time", 1, 0, 'R'},
        {"no-rate-limit", 0, 0, 'u'},  {"secret", 1, 0, 's'},
        {"step-size", 1, 0, 'S'},      {"window-size", 1, 0, 'w'},
        {"minimal-window", 0, 0, 'W'}, {0, 0, 0, 0}};
    idx = -1;
