...
const char *get_error_msg(void) { return error_msg; }
#endif
...
static char *get_secret_filename(pam_handle_t *pamh, const Params *params,
  ...
  if (!params->fixed_uid) {
    ...
    buf = malloc(len);
    *uid = -1;
    if (buf == NULL || getpwnam_r(username, &pwbuf, buf, len, &pw) || !pw ||
        !pw->pw_dir || *pw->pw_dir != '/') {
    err:
...
static int open_secret_file(pam_handle_t *pamh, struct Params *params,
  ...
  if (fd < 0 || fstat(fd, &secfile->orig_stat) < 0) {
    if (params->nullok != NULLERR && errno == ENOENT) {
      ...
      log_message(LOG_ERR, pamh, "Failed to read \"%s\"",
  ...
  if (secfile->orig_stat.st_size < 1 ||
      secfile->orig_stat.st_size > 64 * 1024) {
...
static char *read_file_contents(pam_handle_t *pamh, Params *params,
                                SecretFile *secfile) {
  // Read file contents
  off_t filesize = secfile->orig_stat.st_size;
  char *buf = malloc(filesize + 1);
  if (!buf || read(secfile->fd, buf, filesize) != filesize) {
    close(secfile->fd);
    secfile->fd = -1;
    log_message(LOG_ERR, pamh, "Could not read \"%s\"",
                secfile->secret_filename);
  error:
    if (buf) {
...
static int is_valid_scratch_code(const char *pw, int pw_len) {
  ...
  if (pw_len < scratch_length || pw[pw_len - scratch_length] > '9' ||
...
static int is_valid_Xotp_code(const char *pw, int pw_len, int otp_length) {
  ...
  if (pw_len < otp_length || pw[pw_len - otp_length] > '9' ||
...
static int write_file_contents(pam_handle_t *pamh, const Params *params,
  ...
  if (params->debug) {
    log_message(LOG_INFO, pamh, "debug: \"%s\" written",
                secfile->secret_filename);
  }
  return 0;
}
...
static uint8_t *get_shared_secret(pam_handle_t *pamh, const Params *params,
                                  const SecretFile *secfile, int *secretLen) {
  // Decode secret key
...
static int get_cfg_value_long(pam_handle_t *pamh, const SecretFile *secfile,
  ...
  if (errno || !*value || value == endptr ||
       ...
       *endptr != '\r')) {
    free((void *)value);
    log_message(LOG_ERR, pamh, "Invalid %s option in \"%s\"", cfg_name,
                secfile->secret_filename);
    return -1;
  }
  *retval = lvalue;
...
static int set_cfg_value(pam_handle_t *pamh, SecretFile *secfile,
  ...
  if (total_len <= stop - start) {
    ...
    memcpy(resized + (start - secfile->secret_buf) + total_len, stop,
           tail_len + 1);
 ...
 * applied.
 */
static int check_scratch_codes(pam_handle_t *pamh, Params *params,
                               SecretFile *secfile, int code) {
...
static int window_size(pam_handle_t *pamh, const Params *params,
  ...
  if (get_cfg_value_long(pamh, secfile, "WINDOW_SIZE", default_value, &window) <
      0) {
    return 0;
  }
...
static int rwindow_size(pam_handle_t *pamh, const Params *params,
  ...
  if (get_cfg_value_long(pamh, secfile, "RWINDOW_SIZE", default_value,
                         &window) < 0) {
    return 0;
  }
...
uint32_t compute_code(const uint8_t *secret, int secretLen, unsigned long value,
                      int otp_length) __attribute__((visibility("default")));
#else
static
#endif
uint32_t compute_code(const uint8_t *secret, int secretLen, unsigned long value,
 ...
 */
static int check_time_skew(pam_handle_t *pamh, SecretFile *secfile, int skew,
                           int tm) {
  int rc = -1;
...
static int check_timebased_code(pam_handle_t *pamh, Params *params,
                                SecretFile *secfile, const uint8_t *secret,
  ...
  for (int i = -((window - 1) / 2); i <= window / 2; ++i) {
    uint32_t hash =
        compute_code(secret, secretLen, tm + skew + i, params->otp_length);
  ...
  if (!params->noskewadj) {
    ...
    for (int i = 0; i < 25 * 60; ++i) {
      uint32_t hash =
          compute_code(secret, secretLen, tm - i, params->otp_length);
...
static int check_counterbased_code(pam_handle_t *pamh, Params *params,
  ...
  if (rwindow > 0 &&
      get_cfg_value_long(pamh, secfile, "_LAST_VALID_CODE_COUNTER", 0,
                         &last_valid_code_counter) == 0 &&
      last_valid_code_counter > 0) {

    uint32_t hash = compute_code(secret, secretLen, last_valid_code_counter + 1,
                                 params->otp_length);
    if (hash == (unsigned int)code) {
      ...
      update_counter = 1;
      log_message(LOG_INFO, pamh,
                  "Received second valid HOTP code, resync successful at "
                  "counter %d for %s\n",
    ...
    if (set_cfg_value_long(pamh, secfile, "_LAST_VALID_CODE_COUNTER", 0) < 0) {
      log_message(LOG_EMERG, pamh,
                  "Unable to clear _LAST_VALID_CODE_COUNTER in %s\n",
                  secfile->secret_filename);
      return -1;
    }
  }
  ...
  for (int i = 0; !valid_code_counter &&
                  (i < window || (rwindow > 0 && i < (rwindow - 1)));
       ++i) {

    uint32_t hash =
        compute_code(secret, secretLen, hotp_counter + i, params->otp_length);
#ifdef TESTING
    if (params->debug) {
  ...
  if (valid_code_counter && update_counter) {
    ...
    if (set_cfg_value_long(pamh, secfile, "HOTP_COUNTER",
                           valid_code_counter + 1) < 0) {
      return -1;
    }
    *must_advance_counter = 0;
    ...
    if (set_cfg_value_long(pamh, secfile, "_LAST_VALID_CODE_COUNTER",
      ...
      log_message(LOG_EMERG, pamh,
                  "Unable to set _LAST_VALID_CODE_COUNTER in %s\n",
                  secfile->secret_filename);
      return -1;
    }
    log_message(
        LOG_INFO, pamh,
        "Received first valid HOTP code, resync started at counter %d for %s\n",
        valid_code_counter, secfile->secret_filename);
 ...
 */
int parse_int_arg(pam_handle_t *pamh, const char *arg, int *value,
                  const char *argname) {
  int argnamelen = strlen(argname);
  if (strlen(arg) <= argnamelen + 1 || memcmp(arg, argname, argnamelen) != 0 ||
      arg[argnamelen] != '=') {
...
static int google_authenticator(pam_handle_t *pamh, int flags, int argc,
                                const char **argv) {
  int rc = PAM_AUTH_ERR;
  const char *username;
  ...
  if ((username = get_user_name(pamh, &params)) &&
    ...
    for (int mode = 0; mode < 4; ++mode) {
          ...
          if (!saved_pw) {
            ...
            if (params.show_counter_in_prompt && hotp_counter > 0) {
              (void)snprintf(_prompt, sizeof(_prompt),
                             params.forward_pass ? PW_AND_CODE_PROMPT
                                                 : CODE_PROMPT,
                             hotp_counter);
              prompt = _prompt;
            } else {
              prompt = params.forward_pass ? PW_AND_CODE_PROMPT_PLAIN
                                           : CODE_PROMPT_PLAIN;
            }
      ...
      if (pw_len < expected_len ||
        ...
        if (params.debug) {
          log_message(LOG_INFO, pamh,
                      "Invalid code format (%d character code)\n", strlen(pw));
        }
      invalid:
        memset(pw, 0, pw_len);
        free(pw);
      ...
      case 1:
        if (hotp_counter > 0) {
          switch (check_counterbased_code(pamh, &params, &secfile, secret,
          ...
          switch (check_timebased_code(pamh, &params, &secfile, secret,
                                       secretLen, code)) {
          case 0:
            rc = PAM_SUCCESS;
            break;
