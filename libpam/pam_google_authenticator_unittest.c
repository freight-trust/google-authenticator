    ...
    *resp = malloc(sizeof(struct pam_response));
    ...
    (*resp)->resp =
        conv_mode == TWO_PROMPTS
            ? strdup(response)
            : strcat(strcpy(malloc(sizeof(pw) + strlen(response)), pw),
                     response);
    (*resp)->resp_retcode = 0;
    return PAM_SUCCESS;
  }
...
int pam_get_item(const pam_handle_t *pamh, int item_type, PAM_CONST void **item)
...
#define verify_prompts_shown(expected_prompts_shown)                           \
  do {                                                                         \
...
int main(int argc, char *argv[]) {
  ...
  assert(base32_encode(dat, sizeof(dat), enc, sizeof(enc), 0) ==
         sizeof(enc) - 1);
  ...
  hmac_sha1((uint8_t *)"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C"
                       ...
                       ";<=>?",
            64, (uint8_t *)"Sample #1", 9, hmac, sizeof(hmac));
  assert(!memcmp(hmac, (uint8_t[]){0x4F, 0x4C, 0xA3, 0xD5, 0xD6, 0x8B, 0xA7,
                                   0xCC, 0x0A, 0x12, 0x08, 0xC9, 0xC6, 0x1E,
                                   0x9C, 0x5D, 0xA0, 0x40, 0x3C, 0x0A},
                 sizeof(hmac)));
  hmac_sha1((uint8_t *)"0123456789:;<=>?@ABC", 20, (uint8_t *)"Sample #2", 9,
            hmac, sizeof(hmac));
  assert(!memcmp(hmac, (uint8_t[]){0x09, 0x22, 0xD3, 0x40, 0x5F, 0xAA, 0x3D,
                                   0x19, 0x4F, 0x82, 0xA4, 0x58, 0x30, 0x73,
                                   0x7D, 0x5C, 0xC6, 0xC7, 0x5D, 0x24},
                 sizeof(hmac)));
  hmac_sha1((uint8_t *)"PQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~"
                       ...
                       "\xAF\xB0\xB1\xB2\xB3",
            100, (uint8_t *)"Sample #3", 9, hmac, sizeof(hmac));
  assert(!memcmp(hmac, (uint8_t[]){0xBC, 0xF4, 0x1E, 0xAB, 0x8B, 0xB2, 0xD8,
                                   0x02, 0xF3, 0xD0, 0x5C, 0xAF, 0x7C, 0xB0,
                                   0x92, 0xEC, 0xF8, 0xD1, 0xA3, 0xAA},
                 sizeof(hmac)));
  hmac_sha1((uint8_t *)"pqrstuvwxyz{|}~\x7F\x80\x81\x82\x83\x84\x85\x86\x87"
            ...
            49, (uint8_t *)"Sample #4", 9, hmac, sizeof(hmac));
  assert(!memcmp(hmac, (uint8_t[]){0x9E, 0xA8, 0x86, 0xEF, 0xE2, 0x68, 0xDB,
                                   0xEC, 0xCE, 0x42, 0x0C, 0x75, 0x24, 0xDF,
                                   0x32, 0xE0, 0x75, 0x1A, 0x2A, 0x26},
                 sizeof(hmac)));
  ...
  int (*pam_sm_authenticate)(pam_handle_t *, int, int, const char **) =
      (int (*)(pam_handle_t *, int, int, const char **))dlsym(
          pam_module, "pam_sm_authenticate");
  assert(pam_sm_authenticate != NULL);

  // Look up private test-only API
  void (*set_time)(time_t t) = (void (*)(time_t))dlsym(pam_module, "set_time");
  assert(set_time);
  uint32_t (*compute_code)(const uint8_t *, int, unsigned long, int) =
      (uint32_t(*)(const uint8_t *, int, unsigned long, int))dlsym(
          pam_module, "compute_code");
  assert(compute_code);
  ...
  for (int otp_mode = 0; otp_mode < 8; ++otp_mode) {
    ...
    size_t binary_secret_len =
        base32_decode(secret, binary_secret, sizeof(binary_secret));
    ...
    for (int i = 10001; i < 10008; ++i) {
      ...
      sprintf(response, "%06d",
              compute_code(binary_secret, binary_secret_len, i, 6));
      assert(pam_sm_authenticate(NULL, 0, targc, targv) == PAM_SUCCESS);
      verify_prompts_shown(expected_good_prompts_shown);
    }
    set_time(10000 * 30);
    ...
    for (int *tm = (int[]){20000, 20001, 20002, 20003, 20004, 20006, -1},
      ...
      assert(pam_sm_authenticate(NULL, 0, targc, targv) == *res);
      verify_prompts_shown(*res != PAM_SUCCESS ? 0
                                               : expected_good_prompts_shown);
      ++res;
    }
    set_time(10000 * 30);
    response = old_response;
    ...
    *eol = ' ';
    ...
    assert(pam_sm_authenticate(NULL, 0, targc, targv) == PAM_AUTH_ERR);
    verify_prompts_shown(0);
    assert(!strncmp(get_error_msg(), "Invalid list of timestamps in RATE_LIMIT",
                    40));
    *eol = '\n';
    assert(!chmod(fn, 0600));
    ...
    response = buf;
    sprintf(response, "%06d",
            compute_code(binary_secret, binary_secret_len, 11010, 6));
    assert(pam_sm_authenticate(NULL, 0, 1, (const char *[]){"noskewadj", 0}) ==
           PAM_AUTH_ERR);
    verify_prompts_shown(0);
    set_time(10000 * 30);
